...
namespace OpenFTTH.RouteNetwork.Tests
{
    public class InterestQueryTests : IClassFixture<TestRouteNetwork>
    {
        ...
        public async void QueryReferencesFromRouteElementAndInterestObjects_ShouldReturnAllInterestInformation()
        {
            ...
            var routeNetworkQuery = new GetRouteNetworkDetails(new RouteNetworkElementIdList() { testNetwork.CO_1, testNetwork.S1, testNetwork.HH_1, testNetwork.HH_2 })
            {
            ...
            Assert.Contains(testNetwork.CO_1, queryResult.Value.Interests[interestId].RouteNetworkElementRefs);
            Assert.Contains(testNetwork.S1, queryResult.Value.Interests[interestId].RouteNetworkElementRefs);
            Assert.Contains(testNetwork.HH_1, queryResult.Value.Interests[interestId].RouteNetworkElementRefs);

            // Assert that route element 1 (CO_1) has interest information with correct relation type
            Assert.NotNull(queryResult.Value.RouteNetworkElements[testNetwork.CO_1].InterestRelations);
            Assert.Contains(queryResult.Value.RouteNetworkElements[testNetwork.CO_1].InterestRelations, r => r.RefId == interestId && r.RelationKind == RouteNetworkInterestRelationKindEnum.Start);

            // Assert that route element 2 (S1) has interest information with correct relation type
            Assert.NotNull(queryResult.Value.RouteNetworkElements[testNetwork.S1].InterestRelations);
            Assert.Contains(queryResult.Value.RouteNetworkElements[testNetwork.S1].InterestRelations, r => r.RefId == interestId && r.RelationKind == RouteNetworkInterestRelationKindEnum.PassThrough);

            // Assert that route element 3 (HH_1) has interest information with correct relation type
            Assert.NotNull(queryResult.Value.RouteNetworkElements[testNetwork.HH_1].InterestRelations);
            Assert.Contains(queryResult.Value.RouteNetworkElements[testNetwork.HH_1].InterestRelations, r => r.RefId == interestId && r.RelationKind == RouteNetworkInterestRelationKindEnum.End);

            // Assert that route element 4 (HH_2) has no references to interest information
            Assert.Empty(queryResult.Value.RouteNetworkElements[testNetwork.HH_2].InterestRelations);
        }
        ...
        public async void QueryExplicitlyRequestingNoInterestInformation_ShouldReturnNoInterestInformation()
        {
            ...
            Assert.Null(queryResult.Value.RouteNetworkElements[testNetwork.CO_1].InterestRelations);
            Assert.Null(queryResult.Value.RouteNetworkElements[testNetwork.S1].InterestRelations);
            Assert.Null(queryResult.Value.RouteNetworkElements[testNetwork.HH_1].InterestRelations);
            Assert.Null(queryResult.Value.RouteNetworkElements[testNetwork.HH_2].InterestRelations);
        }
    }
}
